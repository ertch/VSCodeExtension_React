...
describe('CodeGenerator', () => {
    let generator;
    beforeEach(() => {
        generator = new CodeGenerator_1.CodeGenerator();
    });
    describe('generate()', () => {
        it('should generate HTML for simple entity', () => {
            const result = generator.generate(entities_1.TEST_ENTITIES.simpleEntity, { validate: false });
        ...
        it('should generate HTML for nested entities', () => {
            const result = generator.generate(entities_1.TEST_ENTITIES.nestedEntity, { validate: false });
        ...
        it('should handle TabPage entities', () => {
            const result = generator.generate(entities_1.TEST_ENTITIES.tabPage, { validate: false });
        ...
        it('should handle actions (Triple_List)', () => {
            const result = generator.generate(entities_1.TEST_ENTITIES.entityWithActions, { validate: false });
        ...
        it('should handle firstOption (Double_Single)', () => {
            const result = generator.generate(entities_1.TEST_ENTITIES.complexEntity, { validate: false });
        ...
        it('should apply wrapper config', () => {
            const wrapperConfig = {
                before: () => '<!-- Before -->',
                after: () => '<!-- After -->',
                wrapAll: (html) => `<wrapper>\n${html}\n</wrapper>`
            };
            const result = generator.generate(entities_1.TEST_ENTITIES.simpleEntity, {
                wrapperConfig,
                validate: false
            });
        ...
        it('should throw ValidationError for invalid entity when validate=true', () => {
            expect(() => {
                generator.generate(entities_1.TEST_ENTITIES.invalidEntity, { validate: true });
            }).toThrow(ExtensionErrors_1.ValidationError);
        });
        it('should skip validation when validate=false', () => {
            const result = generator.generate(entities_1.TEST_ENTITIES.invalidEntity, { validate: false });
        ...
        it('should handle array of entities', () => {
            const entities = [
                ...
                entities_1.TEST_ENTITIES.nestedEntity
            ];
    ...
    describe('backwards compatibility', () => {
        it('should support deprecated generateHTML function', () => {
            ...
            const result = generateHTML(entities_1.TEST_ENTITIES.simpleEntity);
            expect(result.html).toContain('<Button');
            expect(result.components).toContain('Button');
        });
    });
