...
const tree_utils_1 = require("../../src/utils/tree-utils");
describe('tree-utils', () => {
    describe('genId', () => {
        it('should generate unique IDs', () => {
            ...
            for (let i = 0; i < count; i++) {
                ids.add((0, tree_utils_1.genId)());
            }
            expect(ids.size).toBe(count);
        });
        it('should generate IDs with n_ prefix', () => {
            const id = (0, tree_utils_1.genId)();
    ...
    describe('cloneDeep', () => {
        it('should create deep copy of simple object', () => {
            ...
            const clone = (0, tree_utils_1.cloneDeep)(original);
        ...
        it('should clone TreeNode correctly', () => {
            ...
            const clone = (0, tree_utils_1.cloneDeep)(node);
    ...
    describe('findNodeAndParent', () => {
        ...
        it('should find root node', () => {
            ...
            const result = (0, tree_utils_1.findNodeAndParent)(tree, 'root1');
        ...
        it('should find nested child node', () => {
            var _a;
            const tree = createTestTree();
            const result = (0, tree_utils_1.findNodeAndParent)(tree, 'child1');
            expect(result).not.toBeNull();
            expect(result.node.id).toBe('child1');
            expect((_a = result.parent) === null || _a === void 0 ? void 0 : _a.id).toBe('root1');
            expect(result.index).toBe(0);
        });
        it('should find deeply nested node', () => {
            var _a;
            const tree = createTestTree();
            const result = (0, tree_utils_1.findNodeAndParent)(tree, 'grandchild1');
            expect(result).not.toBeNull();
            expect(result.node.id).toBe('grandchild1');
            expect((_a = result.parent) === null || _a === void 0 ? void 0 : _a.id).toBe('child1');
            expect(result.index).toBe(0);
        });
        it('should return null for non-existent node', () => {
            const tree = createTestTree();
            const result = (0, tree_utils_1.findNodeAndParent)(tree, 'nonexistent');
            expect(result).toBeNull();
        });
        it('should find second root node', () => {
            const tree = createTestTree();
            const result = (0, tree_utils_1.findNodeAndParent)(tree, 'root2');
            expect(result).not.toBeNull();
        ...
        it('should handle empty tree', () => {
            const result = (0, tree_utils_1.findNodeAndParent)([], 'any');
            expect(result).toBeNull();
        });
    });
    describe('removeNode', () => {
        it('should remove root node', () => {
            ...
            const removed = (0, tree_utils_1.removeNode)(tree, 'root1');
        ...
        it('should remove nested child node', () => {
            ...
            const removed = (0, tree_utils_1.removeNode)(tree, 'child1');
            expect(removed).not.toBeNull();
        ...
        it('should remove deeply nested node', () => {
            ...
            const removed = (0, tree_utils_1.removeNode)(tree, 'grandchild');
            expect(removed).not.toBeNull();
        ...
        it('should return null for non-existent node', () => {
            ...
            const removed = (0, tree_utils_1.removeNode)(tree, 'nonexistent');
            expect(removed).toBeNull();
        });
        it('should handle empty tree', () => {
            const tree = [];
            const removed = (0, tree_utils_1.removeNode)(tree, 'any');
            expect(removed).toBeNull();
        });
        it('should not modify tree when node not found', () => {
            ...
            (0, tree_utils_1.removeNode)(tree, 'nonexistent');
    ...
    describe('isDescendant', () => {
        ...
        it('should return true for direct child', () => {
            const tree = createTestTree();
            expect((0, tree_utils_1.isDescendant)(tree, 'child1', 'root')).toBe(true);
        });
        it('should return true for grandchild', () => {
            const tree = createTestTree();
            expect((0, tree_utils_1.isDescendant)(tree, 'grandchild1', 'root')).toBe(true);
        });
        it('should return true for nested descendant', () => {
            const tree = createTestTree();
            expect((0, tree_utils_1.isDescendant)(tree, 'grandchild1', 'child1')).toBe(true);
        });
        it('should return false for sibling', () => {
            const tree = createTestTree();
            expect((0, tree_utils_1.isDescendant)(tree, 'child2', 'child1')).toBe(false);
        });
        it('should return false for parent', () => {
            const tree = createTestTree();
            expect((0, tree_utils_1.isDescendant)(tree, 'root', 'child1')).toBe(false);
        });
        it('should return false for self', () => {
            const tree = createTestTree();
            expect((0, tree_utils_1.isDescendant)(tree, 'child1', 'child1')).toBe(false);
        });
        it('should return false when ancestor not found', () => {
            const tree = createTestTree();
            expect((0, tree_utils_1.isDescendant)(tree, 'child1', 'nonexistent')).toBe(false);
        });
        it('should return false when child not found', () => {
            const tree = createTestTree();
            expect((0, tree_utils_1.isDescendant)(tree, 'nonexistent', 'root')).toBe(false);
        });
        it('should handle empty tree', () => {
            expect((0, tree_utils_1.isDescendant)([], 'any', 'any')).toBe(false);
        });
    });
    describe('insertNode', () => {
        const createNode = (id, canHaveChildren = false) => ({
            id,
            type: 'Test',
            canHaveChildren,
            props: {},
        ...
        describe('root level insertion', () => {
            it('should insert at root when targetId is null', () => {
                const tree = [createNode('existing')];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, null, 'above', newNode);
                expect(tree.length).toBe(2);
                expect(tree[1].id).toBe('new');
            });
            it('should insert above target at root', () => {
                const tree = [createNode('node1'), createNode('node2')];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'node2', 'above', newNode);
            ...
            it('should insert below target at root', () => {
                const tree = [createNode('node1'), createNode('node2')];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'node1', 'below', newNode);
                expect(tree.length).toBe(3);
                expect(tree[0].id).toBe('node1');
                expect(tree[1].id).toBe('new');
                expect(tree[2].id).toBe('node2');
        ...
        describe('inside insertion', () => {
            it('should insert inside container node', () => {
                const tree = [createNode('container', true)];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'container', 'inside', newNode);
                expect(tree[0].children.length).toBe(1);
                expect(tree[0].children[0].id).toBe('new');
            });
            it('should not insert inside non-container node (ignores insert)', () => {
                const tree = [createNode('noncontainer', false)];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'noncontainer', 'inside', newNode);
            ...
            it('should insert inside nested container', () => {
                const tree = [
                    Object.assign(Object.assign({}, createNode('root', true)), { children: [createNode('child', true)] }),
                ];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'child', 'inside', newNode);
        ...
        describe('nested insertion', () => {
            it('should insert above nested node', () => {
                const tree = [
                    Object.assign(Object.assign({}, createNode('root', true)), { children: [createNode('child1'), createNode('child2')] }),
                ];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'child2', 'above', newNode);
            ...
            it('should insert below nested node', () => {
                ...
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'child1', 'below', newNode);
                expect(tree[0].children.length).toBe(3);
                expect(tree[0].children[0].id).toBe('child1');
                expect(tree[0].children[1].id).toBe('new');
                expect(tree[0].children[2].id).toBe('child2');
        ...
        describe('edge cases', () => {
            it('should handle non-existent target gracefully', () => {
                const tree = [createNode('existing')];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'nonexistent', 'above', newNode);
            ...
            it('should insert at beginning when above first element', () => {
                const tree = [createNode('first'), createNode('second')];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'first', 'above', newNode);
            ...
            it('should insert at end when below last element', () => {
                const tree = [createNode('first'), createNode('last')];
                const newNode = createNode('new');
                (0, tree_utils_1.insertNode)(tree, 'last', 'below', newNode);
